...
export default function EmployeeDetail({ emp: initEmp, onBack, settings }: { emp: Employee; onBack: () => void; settings: Settings }) {
  ...
  return <div className="animate-fade-slide-up" style={{ padding: 32, maxWidth: 1000, margin: "0 auto" }}>
    ...
    {tab === "overview" && <div style={{ display: "flex", flexDirection: "column" as const, gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${t.border}`, padding: 24 }}><h3 style={{ fontSize: 15, fontWeight: 650, color: t.text, margin: "0 0 20px" }}>Personal Information</h3>{[{l:"Employee ID",v:emp.eid},{l:"Date of Birth",v:emp.dob ? (() => { const parts = emp.dob.split("-"); const m = parseInt(parts[1])-1; const dy = parseInt(parts[2]); const yr = parseInt(parts[0]); const months = ["January","February","March","April","May","June","July","August","September","October","November","December"]; const age = Math.floor((Date.now() - new Date(yr,m,dy).getTime()) / (365.25*24*60*60*1000)); const today = new Date(); const isToday = m === today.getMonth() && dy === today.getDate(); return `${months[m]} ${dy}, ${yr} (${age}y)` + (isToday ? " 🎂" : ""); })() : "—"},{l:"Contract Type",v:emp.contract},{l:"Hire Date",v:fmtDate(emp.hire)}].map((f,i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: i < 3 ? `1px solid ${t.borderLight}` : "none" }}><span style={{ fontSize: 13, color: t.textSecondary }}>{f.l}</span><span style={{ fontSize: 13, color: t.text, fontWeight: 550 }}>{f.v}</span></div>)}</div>
